# 每次排序调用 `.sort()` 方法时，你都在使用世界上最快的排序算法 Timsort
...
算法直接跳跃到位置 15 (7 + 2^3)，发现 `30` 大于 `25`，进而在位置 7 和 15 之间执行二分搜索，以找到 `25` 的插入点。

![动图 跃进模式](/doc/illustrations/timsort/Timsort09.gif)
