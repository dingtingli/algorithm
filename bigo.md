...
最近正好在看一些介绍算法的书籍，自然也是绕不过这个概念的。这次我抱着试试心态，想搞明白大 O 到底是什么意思，最终还真得到了一些收获。

大 O 是一个数学上的概念，算法分析复杂度的时候借用了这个概念。

这篇文章我们就从数学上的大 O 开始介绍，看看它是如何演变成分析算法复杂度的。
...
三个函数都在增大，增大的趋势其实就是速度变化的趋势，所以 y1 和 y2 可以是一类趋势（匀速运动），y3 是另一类趋势（加速运动）。
...
可以看到 x 等于 100 的时候，对 y 结果影响最大的还是 `1000x^2`。

当 x 等于 1百万 时，3x^3 以外的其他结果都可以忽略了。

从这个变化趋势我们可以看出，当 x 趋于无穷大时，函数的结果只跟 x 最大次方的元素有关。

所以其实不需要计算，直接就可以看出函数 `y = 3x^3 - 1000x^2 + 5x -7` 的大 O 为 O(x^3)。

这也进一步验证了之前的结论，大 O 能即简洁又快速地找出同阶函数。